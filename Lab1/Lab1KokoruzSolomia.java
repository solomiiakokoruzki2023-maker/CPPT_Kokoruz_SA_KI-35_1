 ...
 */
public class Lab1KokoruzSolomia {
    /**
    ...
    public static void main(String[] args) {
        ...
        // Перевірка на валідність символа заповнювача (символ повинен бути тільки один )
        if (symbol.length() != 1) {
            System.out.println("Введіть коректний символ заповнювач");
            return;
        }

        String[][] arr = createLengthOfEachSubArr(row); // створює зубчастий масив
        String fileName = "Lab1.txt"; 

        // Запуск бізнес логіки для генерації зубчастого масиву (виводу в консоль і запису в файл)
        try {
            printMatrix(arr, symbol, row, fileName); // метод який заповнює матрицю символами у формі зубців 
        } catch (IOException e) { 
